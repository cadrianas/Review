...
abstractive_model = T5ForConditionalGeneration.from_pretrained("t5-small")
abstractive_tokenizer = T5Tokenizer.from_pretrained("t5-small")

# Step 4: Combined Extractive and Abstractive Summarization function
def combined_summarization_t5_small(document, top_n=10):
    # Handle NaN values
    if pd.isna(document):
...
data['summary_t5_small'] = data['abstract'].apply(combined_summarization_t5_small)

# Step 6: Display or save the results
data.to_csv('t5_small_summary.csv', index=False)
print("Summaries saved to 't5_small_summary.csv'.")