...
data = pd.read_csv('time_dependent.csv')
...
abstractive_model = T5ForConditionalGeneration.from_pretrained("t5-large")
abstractive_tokenizer = T5Tokenizer.from_pretrained("t5-large")

# Step 4: Combined Extractive and Abstractive Summarization function
def combined_summarization_t5_large(document, top_n=10):
    # Handle NaN values
    if pd.isna(document):
...
data['summary_t5_large'] = data['abstract'].apply(combined_summarization_t5_large)

# Step 6: Display or save the results
data.to_csv('time_dependent_t5_large_summary.csv', index=False)
print("Summaries saved to 'time_dependent_t5_large_summary.csv'.")